...
void CRFIDDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT2, m_check);
	DDX_Control(pDX, IDC_BUTTON6, m_borrow);
}
...
BEGIN_MESSAGE_MAP(CRFIDDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON6, &CRFIDDlg::OnClickedButton6)
END_MESSAGE_MAP()
...
void CRFIDDlg::OnReadOnce()
{
	...
	if (flag_r == 0)
	{
		...
		CString Path_img = _T("img\\img1.bmp");
		...
		dc.BitBlt(40, 40, 320, 240, &memDC, 0, 0, SRCCOPY);
...
void CRFIDDlg::OnReadContinue()
{
	...
	if (flag_r == 0)
	{
		...
		CBitmap m_bitmap;
		//CString Path_img = "img\\null.bmp";
		CString Path_img = _T("img\\img2.bmp");
		m_bitmap.DeleteObject();
		m_bitmap.Attach((HBITMAP)LoadImage(NULL, Path_img, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE));
		CDC* pDC = GetDC();
		CDC memDC;
...
void CRFIDDlg::OnBnClickedButton5()
{
	...
	CBitmap m_bitmap;
	//CString Path_img = "img\\null.bmp";
	CString Path_img = _T("img\\img3.bmp");
	m_bitmap.DeleteObject();
	m_bitmap.Attach((HBITMAP)LoadImage(NULL, Path_img, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE));
	CDC* pDC = GetDC();
	CDC memDC;
	...
	dc.BitBlt(30, 30, 320, 240, &memDC, 0, 0, SRCCOPY);
...
void CRFIDDlg::OnClickedButton6()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CString chk_un = _T("대출중");
	m_check = chk_un;
